...
export const HierarchyNode = memo<HierarchyNodeProps>(
  ({
    ...
    const [setMode, sourceSetId, spaceId] = useContentManagerStore((s) => [
      s.setMode,
      s.sourceSetId,
      s.spaceId,
    ]);

    const [renameFolder, updateContentItem, refreshFileList] = useFileStore((s) => [
      s.renameFolder,
      s.updateContentItem,
      s.refreshFileList,
    ]);
    ...
    const handleRenameStart = useCallback(() => {
      ...
      setRenamingValue(item.isFolder || isPage ? item.name : baseName);
      // Focus input after render
      setTimeout(() => {
        inputRef.current?.focus();
        inputRef.current?.select();
      }, 0);
    }, [item.name, item.isFolder, isPage, baseName]);

    const handleRenameConfirm = useCallback(async () => {
      if (!renamingValue.trim()) {
        message.error(t('actions.rename.empty'));
        return;
      }

      // For files, append extension back; for folders/pages, use the value as-is
      const finalName =
        item.isFolder || isPage ? renamingValue.trim() : renamingValue.trim() + extension;
      ...
      try {
        ...
        if (item.isFolder) {
          ...
          await updateContentItem(item.id, { name: finalName });
          await refreshFileList();
        }
        if (sourceSetId) {
          await clearTreeFolderCache(sourceSetId);
        }
        message.success(t('actions.rename.success'));
        setIsRenaming(false);
      } catch (error) {
        console.error('Rename error:', error);
        message.error(t('actions.rename.failed'));
      }
    }, [
      item.id,
      item.name,
    ...
    const handleFolderClick = useCallback(
      (folderId: string, folderSlug?: string | null) => {
        ...
        if (sourceSetId) {
          navigate(buildSourceSetFolderPath(spaceId, sourceSetId, navKey));
        }

        setMode('explorer');
      },
      [sourceSetId, navigate, setMode, spaceId],
    );
