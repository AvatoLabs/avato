...
export { clearTreeFolderCache, clearTreeStateForSourceSet } from './treeState';

/**
 * As a sidebar along with the Explorer
 */
const SourceSetTree = memo(() => {
  ...
  const [sourceSetId, currentViewItemId] = useContentManagerStore((s) => [
    s.sourceSetId,
    s.currentViewItemId,
  ]);
  const [spaceId, currentFolderId] = useContentManagerStore((s) => [s.spaceId, s.currentFolderId]);
  ...
  const state = useMemo(() => getTreeState(sourceSetId || ''), [sourceSetId]);
  const { expandedFolders, folderChildrenCache, loadingFolders } = state;

  // Fetch breadcrumb for current folder
  const { data: folderBreadcrumb } = useFetchFolderBreadcrumb(currentFolderSlug, spaceId);
  ...
  const isExplorerCacheActiveForTree = useMemo(() => {
    ...
    return resourceQueryParams.sourceSetId === sourceSetId;
  }, [sourceSetId, resourceQueryParams]);

  const explorerParentKey = useMemo(() => {
    if (!isExplorerCacheActiveForTree) return null;
    // Explorer route params are slug-based; tree state must stay keyed by document id.
    return currentFolderId ?? null;
  }, [currentFolderId, isExplorerCacheActiveForTree]);

  const explorerChildren = useMemo(() => {
    if (!isExplorerCacheActiveForTree) return [];
    return sortTreeItems(resourceList.map(contentItemToTreeItem));
  ...
  useEffect(() => {
    ...
    const handleTreeRefresh = (event: Event) => {
      const detail = (event as CustomEvent<{ sourceSetId?: string }>).detail;
      if (detail?.sourceSetId && sourceSetId && detail.sourceSetId !== sourceSetId) return;
      forceUpdate();
    };
  ...
  useEffect(() => {
    ...
    if (currentFile.parentId) {
      ...
      const fetchParentPath = async () => {
        try {
          const parentBreadcrumb = await fileService.getFolderBreadcrumb(
            currentFile.parentId!,
            currentFile.spaceId ?? undefined,
          );
...
SourceSetTree.displayName = 'FileTree';

export default SourceSetTree;
