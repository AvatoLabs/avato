...
const FileListItem = memo<FileListItemProps>(
  ({
    ...
    // Consolidate all ContentManagerStore subscriptions with shallow equality
    const contentManagerState = useContentManagerStore(
      (s) => ({
    ...
    const handleDragStart = useCallback(
      (e: DragEvent) => {
        if (!contentManagerState.sourceSetId) {
          e.preventDefault();
          return;
        }
      ...
      [contentManagerState.sourceSetId, dragData, id, isFolder, setCurrentDrag],
    );
    ...
    const handleRenameStart = useCallback(() => {
      setIsRenaming(true);
      // For files (not folders or pages), edit only the base name without extension
      setRenamingValue(isFolder || isPage ? name : baseName);
      // Focus input after render
      setTimeout(() => {
        inputRef.current?.focus();
        inputRef.current?.select();
      }, 0);
    }, [name, baseName, isFolder, isPage]);
    ...
    const handleRenameConfirm = useCallback(async () => {
      ...
      const finalName =
        isFolder || isPage ? renamingValue.trim() : renamingValue.trim() + extension;
      ...
      try {
        // Use optimistic updateContentItem for instant UI update
        await fileStoreState.updateContentItem(id, { name: finalName });
        if (contentManagerState.sourceSetId) {
          await clearTreeFolderCache(contentManagerState.sourceSetId);
        }
    ...
    useEffect(() => {
      if (pendingRenameItemId === id && !isRenaming) {
        handleRenameStart();
        contentManagerState.setPendingRenameItemId(null);
      }
