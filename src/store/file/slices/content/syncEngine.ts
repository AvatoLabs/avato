...
import { type ContentItem, type SyncOperation } from '@/types/content';
...
export class ResourceSyncEngine {
  ...
  private async processOperation(operation: SyncOperation): Promise<void> {
    ...
    try {
      ...
      switch (type) {
        case 'create': {
          const created = await contentService.createContentItem(payload);
        ...
        case 'update': {
          const updated = await contentService.updateContentItem(resourceId, payload);
          this.updateResourceInStore(updated);
          break;
        }

        case 'delete': {
          await contentService.deleteContentItem(resourceId);
          // Resource already removed from store optimistically
          break;
        }

        case 'move': {
          await contentService.moveContentItem(resourceId, payload.parentId);
   ...
   */
  private replaceTempResource(tempId: string, realResource: ContentItem): void {
   ...
   */
  private updateResourceInStore(resource: ContentItem): void {
    const { resourceMap, resourceList } = this.stateManager.getState();
