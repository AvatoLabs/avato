...
export const createRagEvalDatasetSlice = (set: Setter, get: () => SourceSetStore, _api?: unknown) =>
...
export class RAGEvalDatasetActionImpl {
  ...
  useFetchDatasets = (sourceSetId: string): SWRResponse<RAGEvalDataSetItem[]> => {
    return useClientDataSWR<RAGEvalDataSetItem[]>(
      [FETCH_DATASET_LIST_KEY, sourceSetId],
      () => ragEvalService.getDatasets(sourceSetId),
      {
        fallbackData: [],
        onSuccess: () => {
