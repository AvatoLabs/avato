...
export const getDefaultTableColumnName = (index: number) =>
  index === 0
    ? translateTableCopy('docEditor.table.primaryColumnName', DEFAULT_FIRST_COLUMN_NAME)
    : translateTableCopy(
        'docEditor.table.defaultColumnName',
        `${DEFAULT_COLUMN_PREFIX} ${index + 1}`,
        { index: index + 1 },
      );

const getLegacyBuggedLocalizedDefaultColumnName = (index: number) =>
  translateTableCopy('docEditor.table.defaultColumnName', `${DEFAULT_COLUMN_PREFIX} ${index}`, {
    index,
  });
...
const isTableLine = (line: string) => {
  ...
  const cellCount = splitMarkdownRow(trimmed).length;
  if (trimmed.startsWith('|') && trimmed.endsWith('|')) return cellCount >= 1;
...
const normalizeColumns = (headers: string[], fallbackCount = DEFAULT_TABLE_COLUMNS) => {
  ...
  return normalizeAutoGeneratedColumns(columns);
};

export const isRecognizedAutoGeneratedTableColumnName = (name: string, fieldCount: number) => {
  const trimmed = name.trim();
  if (!trimmed) return false;
  if (/^Column \d+$/.test(trimmed)) return true;

  for (let index = 0; index <= fieldCount; index += 1) {
    if (trimmed === getDefaultTableColumnName(index)) return true;
  }

  for (let index = 1; index <= fieldCount; index += 1) {
    if (trimmed === getLegacyBuggedLocalizedDefaultColumnName(index)) return true;
  }

  return false;
};

const normalizeAutoGeneratedColumns = (columns: TableSheetColumn[]) => {
  if (columns.length <= 1) return columns;

  const secondaryColumns = columns.slice(1);
  if (
    !secondaryColumns.every((column) =>
      isRecognizedAutoGeneratedTableColumnName(column.name, columns.length),
    )
  )
    return columns;
  ...
  return columns.map((column, index) => ({
    ...column,
    name:
      index === 0 ? localizeLegacyColumnName(column.name, index) : getDefaultTableColumnName(index),
  }));
};
