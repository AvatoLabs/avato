...
export const DndContextWrapper = memo<PropsWithChildren>(({ children }) => {
  ...
  const selectedFileIds = useContentManagerStore((s) => s.selectedFileIds);
  const setSelectedFileIds = useContentManagerStore((s) => s.setSelectedFileIds);
  ...
  useEffect(() => {
    ...
    const handleDrop = async (event: DragEvent) => {
      ...
      try {
        // Move all items using optimistic moveContentItem
        const pools = itemsToMove.map((id) => moveContentItem(id, targetParentId));

        await Promise.all(pools);

        // Refetch resources to update the view (items should disappear from current folder)
        const { revalidateResources } = await import('@/store/file/slices/content/hooks');
