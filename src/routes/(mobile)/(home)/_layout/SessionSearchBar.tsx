...
const SessionSearchBar = memo<{ mobile?: boolean }>(({ mobile }) => {
  ...
  const debouncedKeywords = useDebounce(keywords, { wait: SESSION_SEARCH_DEBOUNCE });
  const isSearchPending = keywords !== debouncedKeywords;
  ...
  return (
    <SearchBar
      ...
      loading={!isLoaded || isSearchPending}
