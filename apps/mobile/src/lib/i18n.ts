...
const en: TranslationKeys = {
  ...
  serverTips: '💡 Make sure your phone and server are on the same network. The server should be running LobeHub with the tRPC mobile endpoint enabled (/trpc/mobile).',
...
const zh_tw: TranslationKeys = {
  ...
  serverTips: '💡 請確保手機和伺服器在同一網路中。伺服器需要執行 LobeHub 並啟用 tRPC 行動端點 (/trpc/mobile)。',
...
const zh: TranslationKeys = {
  ...
  serverTips: '💡 请确保手机和服务器在同一网络中。服务器需要运行 LobeHub 并启用 tRPC 移动端点 (/trpc/mobile)。',
