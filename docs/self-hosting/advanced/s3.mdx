...
<Steps>
  ...
  Legacy opt-in flag read from the environment. **Default is off** (unset or any value other than `1`). The application’s S3 uploads do **not** set per-object ACLs; objects rely on the bucket default and reads use presigned URLs or the `/f/:id` proxy. Set `S3_SET_ACL=1` only if external tooling still expects this variable to be enabled.
