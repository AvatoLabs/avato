...
- Type: Optional
- Description: Shared-secret headers required by user-memory webhooks such as `/api/webhooks/memory-extraction`. Use `Header=Value` pairs separated by commas.
- Default: -
- Example: `Authorization=Bearer internal-memory-secret` or `x-memory-secret=super-secret`

Use this when calling the memory extraction webhooks from an external scheduler or worker. LobeHub will compare every configured header exactly.

### `MEMORY_USER_MEMORY_WEBHOOK_ALLOW_INSECURE_DEV`

- Type: Optional
- Description: Allows unauthenticated user-memory webhooks only outside production. This is intended strictly for local development.
- Default: `false`
- Example: `true`

<Callout type="warning">
  This bypass is ignored in production. In production, configure
  `MEMORY_USER_MEMORY_WEBHOOK_HEADERS` or use internal service auth instead.
</Callout>
...
- Type: Optional
- Description: Base URL for the **plugin index / registry** used by the plugin gateway and related server flows (not the Community Discover catalog). If you self-host that index, set this to your endpoint. **Community Discover** plugin and MCP lists are served from the **Market** backend (see `MARKET_BASE_URL` / market configuration), not from this URL.
- Default: `https://chat-plugins.lobehub.com`

For how Discover plugin data is loaded vs this variable, see `docs/development/discover-plugin-market.md`.
