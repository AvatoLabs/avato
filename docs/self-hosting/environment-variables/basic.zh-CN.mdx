...
- 类型：可选
- 描述：为 `/api/webhooks/memory-extraction` 等 user-memory webhook 配置共享密钥请求头，格式为 `Header=Value`，多个键值对用英文逗号分隔。
- 默认值：-
- 示例：`Authorization=Bearer internal-memory-secret` 或 `x-memory-secret=super-secret`

当你从外部调度器或 worker 调用记忆抽取 webhook 时，建议使用这个变量。LobeHub 会对每个已配置的 header 做精确匹配。

### `MEMORY_USER_MEMORY_WEBHOOK_ALLOW_INSECURE_DEV`

- 类型：可选
- 描述：仅在非生产环境下允许无鉴权调用 user-memory webhook。这个开关只用于本地开发。
- 默认值：`false`
- 示例：`true`

<Callout type="warning">
  该开关在生产环境中无效。生产环境仍必须配置
  `MEMORY_USER_MEMORY_WEBHOOK_HEADERS` 或使用内部服务鉴权。
</Callout>
...
- 类型：可选
- 描述：用于配置 LobeHub 默认智能体的默认配置。它支持多种数据类型和结构，包括键值对、嵌套字段、数组值等。
- 默认值：`-`
- 示例：`'model=gpt-4-1106-preview;params.max_tokens=300;plugins=search-engine,lobe-image-designer'`

`DEFAULT_AGENT_CONFIG` 用于配置 LobeHub 默认智能体的默认配置。它支持多种数据类型和结构，包括键值对、嵌套字段、数组值等。下表详细说明了 `DEFAULT_AGENT_CONFIG` 环境变量的配置项、示例以及相应解释：
...
- 类型：可选
- 描述：**插件索引 / 注册表**的基址，供插件 Gateway 及相关服务端逻辑使用（**不是**社区 Discover 插件目录的数据源）。若自建该索引服务，可指向你的地址。**社区 Discover** 中的插件与 MCP 列表来自 **Market** 后端（见 `MARKET_BASE_URL` 等市场相关配置），与本变量无关。
- 默认值：`https://chat-plugins.lobehub.com`

Discover 插件数据路径与本文变量的区别见：`docs/development/discover-plugin-market.zh-CN.md`。
