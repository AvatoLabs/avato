---
title: MCP 市场
description: '为智能体接入 10,000+ 工具——数据库、API、文件系统等。一个协议，无限集成。'
tags:
  - LobeHub
  - MCP
  - 模型上下文协议
...
**模型上下文协议（Model Context Protocol，MCP）** 是一种开放标准，让智能体与外部工具、数据源和服务建立无缝、安全的连接。你可以将其理解为一个通用适配器 —— 智能体能够与数据库、API、文件系统、开发工具、业务应用等几乎任何外部服务交互。

LobeHub 的 MCP 市场汇聚了来自全球创作者的优质 MCP，提供超过 10,000 个现成工具，随时为你的智能体注入能力。

## 什么是 MCP？

传统智能体功能有限 —— 只能聊天，无法与外部工具交互或获取实时数据。MCP 打破了这一壁垒：
...
如果你前面还有 Nginx、Ingress 或 CDN，请把代理读超时设置得略高于应用路由预算，并确保应用路由预算高于 `MCP_TOOL_TIMEOUT`。
