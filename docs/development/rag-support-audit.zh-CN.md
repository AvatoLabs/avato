...
| 工具              | 实现状态 | 说明                               |
| ----------------- | -------- | ---------------------------------- |
| `searchSourceSet` | ✅       | 语义搜索，返回文件摘要与相关 chunk |
| `readSourceFiles` | ✅       | 按 fileIds 读取完整文件内容        |
...
- **知识库**：仅注入知识库元信息，实际检索由 `searchSourceSet` / `readSourceFiles` 完成
...
| 知识库工具渲染   | ✅       | `SearchSourceSet` 结果展示、Streaming 占位   |
