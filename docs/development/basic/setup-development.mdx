...
If you see `Error: column sessions.tag_id does not exist` (e.g. from `session.getGroupedSessions`), the database schema is outdated. Run:
...
Restart the service after migration completes.
