...
### `column sessions.tag_id does not exist`

若出现 `Error: column sessions.tag_id does not exist`（常见于 `session.getGroupedSessions` 调用），说明数据库 schema 未更新。请执行：

```bash
DATABASE_URL=你的数据库连接串 pnpm db:migrate
```

或者如果你使用 Bun：

```bash
DATABASE_URL=你的数据库连接串 bun run db:migrate
```
