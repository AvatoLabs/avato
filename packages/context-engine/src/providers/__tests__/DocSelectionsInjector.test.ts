...
import { DocSelectionsInjector } from '../DocSelectionsInjector';

describe('DocSelectionsInjector', () => {
  ...
  const createDocSelection = (id: string, xmlContent: string, docId = 'doc-1') => ({
    content: xmlContent, // preview content
    id,
    docId,
    xml: xmlContent, // actual content used by formatDocSelections
  });

  describe('enabled/disabled', () => {
    it('should skip injection when disabled', async () => {
      const injector = new DocSelectionsInjector({ enabled: false });

      const context = createContext([
        {
          content: 'Question',
          metadata: {
            docSelections: [createDocSelection('sel-1', 'Selected text')],
          },
          role: 'user',
        },
      ]);
      ...
      expect(result.metadata.DocSelectionsInjectorInjectedCount).toBeUndefined();
    });

    it('should inject when enabled', async () => {
      const injector = new DocSelectionsInjector({ enabled: true });
      ...
      expect(result.messages[0].content).toContain('<user_doc_selections>');
  ...
  describe('injection to every user message', () => {
    it('should inject selections to each user message that has them', async () => {
      ...
      const context = createContext([
        {
          ...
          metadata: {
            docSelections: [createDocSelection('sel-1', 'First selection')],
          },
          role: 'user',
        },
        { content: 'First answer', role: 'assistant' },
        {
          ...
          metadata: {
            docSelections: [createDocSelection('sel-2', 'Second selection')],
          },
          role: 'user',
        },
      ...
      expect(result.messages[2].content).toContain('<user_doc_selections>');
      ...
      expect(result.metadata.DocSelectionsInjectorInjectedCount).toBe(2);
    });
    ...
    it('should skip user messages without docSelections', async () => {
      ...
      const context = createContext([
        ...
        { content: 'Answer', role: 'assistant' },
        {
          ...
          metadata: {
            docSelections: [createDocSelection('sel-1', 'Some text')],
          },
          role: 'user',
        },
      ]);
    ...
    it('should skip user messages with empty docSelections array', async () => {
      const injector = new DocSelectionsInjector({ enabled: true });

      const context = createContext([
        {
          content: 'Empty selections',
          metadata: { docSelections: [] },
          role: 'user',
        },
      ]);
  ...
  describe('SYSTEM CONTEXT wrapper', () => {
    it('should wrap selection content with SYSTEM CONTEXT markers', async () => {
      ...
      expect(content).toContain('<user_doc_selections>');
      expect(content).toContain('</user_doc_selections>');
      ...
      const context = createContext([
        {
          ...
          metadata: {
            docSelections: [
              createDocSelection('sel-1', 'First selection'),
              createDocSelection('sel-2', 'Second selection'),
            ],
          },
          role: 'user',
        },
  ...
  describe('multimodal messages', () => {
    it('should handle array content with text parts', async () => {
      ...
      expect(result.messages[0].content[0].text).toContain('<user_doc_selections>');
  ...
  describe('integration with DocEditorContextInjector', () => {
    it('should create wrapper that DocEditorContextInjector can reuse', async () => {
      const injector = new DocSelectionsInjector({ enabled: true });

      const context = createContext([
        {
          content: 'Question about the doc',
          metadata: {
            docSelections: [createDocSelection('sel-1', 'Selected paragraph')],
          },
          role: 'user',
        },
      ]);
      ...
      const selectionsIndex = content.indexOf('<user_doc_selections>');
  ...
  describe('metadata', () => {
    it('should set metadata when injections are made', async () => {
      ...
      const context = createContext([
        {
          content: 'Question',
          metadata: {
            docSelections: [createDocSelection('sel-1', 'Text')],
          },
          role: 'user',
        },
      ]);
      ...
      expect(result.metadata.DocSelectionsInjectorInjectedCount).toBe(1);
    });
