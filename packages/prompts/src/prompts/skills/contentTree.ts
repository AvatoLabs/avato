...
export const buildContentTreeText = (resources: Record<string, SkillResourceMeta>): string => {
  const tree = buildTree(resources);
  const lines = renderTree(tree);
  return lines.join('\n');
};

export const contentTreePrompt = (
  skillName: string,
  resources: Record<string, SkillResourceMeta>,
): string => {
  const treeText = buildContentTreeText(resources);
