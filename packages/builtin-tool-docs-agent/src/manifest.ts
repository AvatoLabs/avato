...
import { DocApiName, DocsAgentIdentifier } from './types';

export const DocsAgentManifest: BuiltinToolManifest = {
  api: [
    // ============ Initialize ============
    {
      description:
    ...
    {
      description:
        'Get the current doc content and metadata. Returns the document in XML format with node IDs, markdown format, or both. Use this tool to retrieve the latest state of the document.',
      name: DocApiName.getDocContent,
      parameters: {
        properties: {
          format: {
            default: 'both',
