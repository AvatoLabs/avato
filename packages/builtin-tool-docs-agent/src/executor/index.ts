...
import { DocsAgentIdentifier } from '../types';

/**
 * API enum for Docs Agent executor
 * Only includes APIs that are exposed in the manifest
 */
const DocsAgentApiName = {
  // Document Metadata
  editTitle: 'editTitle',
 ...
 *
 * Note: Docs Agent is a client-side tool that directly manipulates the Lexical editor.
 * The runtime must be configured with an editor instance before use.
 */
class DocsAgentExecutor extends BaseExecutor<typeof DocsAgentApiName> {
  readonly identifier = DocsAgentIdentifier;
  protected readonly apiEnum = DocsAgentApiName;
   ...
   */
  initDoc = async (params: InitDocumentArgs): Promise<BuiltinToolResult> => {
    try {
      const result = await this.runtime.initDoc(params);
  ...
  /**
   * Get doc content in XML, markdown, or both formats
   */
  getDocContent = async (params: GetDocContentArgs): Promise<BuiltinToolResult> => {
    try {
      const result = await this.runtime.getDocContent(params);

      const state: GetDocContentState = {
      ...
      // For getDocContent, the content IS the document content
