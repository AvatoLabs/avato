...
AgentGroupRoutes.get(
  '/',
  requireAuth,
  requireAnyPermission(getAllScopePermissions('AGENT_READ'), '您没有权限查看智能体分类列表'),
  async (c) => {
...
AgentGroupRoutes.post(
  '/',
  requireAuth,
  requireAnyPermission(getAllScopePermissions('AGENT_CREATE'), '您没有权限创建智能体分类'),
...
AgentGroupRoutes.get(
  '/:id',
  requireAuth,
  requireAnyPermission(getAllScopePermissions('AGENT_READ'), '您没有权限查看智能体分类详情'),
...
AgentGroupRoutes.patch(
  '/:id',
  requireAuth,
  requireAnyPermission(getAllScopePermissions('AGENT_UPDATE'), '您没有权限更新智能体分类'),
  zValidator('param', AgentGroupIdParamSchema),
 ...
 * 行为说明:
 * - 删除指定的智能体分类
 * - 分类内智能体不会被删除，会自动变为未分类状态（sessionGroupId 设为 null）
 */
AgentGroupRoutes.delete(
  '/:id',
  requireAuth,
  requireAnyPermission(getAllScopePermissions('AGENT_DELETE'), '您没有权限删除智能体分类'),
  zValidator('param', AgentGroupIdParamSchema),
  async (c) => {
    const controller = new AgentGroupController();
