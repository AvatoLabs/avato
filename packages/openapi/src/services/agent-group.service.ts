...
export class AgentGroupService extends BaseService {
  ...
  async getAgentGroups(): ServiceResult<AgentGroupListResponse> {
    this.log('info', '获取智能体分类列表');

    try {
      // 权限校验
      const permissionResult = await this.resolveOperationPermission('AGENT_READ');
      if (!permissionResult.isPermitted) {
        throw this.createAuthorizationError(permissionResult.message || '无权访问智能体分类列表');
      }
      ...
      this.log('info', `查询到 ${agentGroupList.length} 个智能体分类`);

      return agentGroupList;
    } catch (error) {
      this.handleServiceError(error, '获取智能体分类列表');
    }
  }

  /**
   * 根据 ID 获取智能体分类详情
   * @param groupId 智能体分类 ID
   * @returns 智能体分类详情
   */
  async getAgentGroupById(groupId: string): ServiceResult<AgentGroupListResponse[0] | null> {
    try {
      this.log('info', '根据 ID 获取智能体分类详情', { groupId });

      // 权限校验
      const permissionResult = await this.resolveOperationPermission('AGENT_READ');
      if (!permissionResult.isPermitted) {
        throw this.createAuthorizationError(permissionResult.message || '无权访问此智能体分类');
      }
      ...
      if (!agentGroup) {
        this.log('warn', '智能体分类不存在', { groupId });
        return null;
      }

      return agentGroup;
    } catch (error) {
      this.handleServiceError(error, '获取智能体分类详情');
    }
  }
   ...
   * @returns 创建完成的智能体分类 ID
   */
  async createAgentGroup(request: CreateAgentGroupRequest): ServiceResult<string> {
    this.log('info', '创建智能体分类', { name: request.name, sort: request.sort });

    try {
      // 权限校验
      const permissionResult = await this.resolveOperationPermission('AGENT_CREATE');
      if (!permissionResult.isPermitted) {
        throw this.createAuthorizationError(permissionResult.message || '无权创建智能体分类');
      }
      ...
      if (!result) {
        throw this.createBusinessError('智能体分类创建失败');
      }

      this.log('info', '智能体分类创建成功', { id: result.id, name: request.name });
      return result.id;
    } catch (error) {
      this.handleServiceError(error, '创建智能体分类');
    }
  }
  ...
  async updateAgentGroup(request: UpdateAgentGroupRequest): ServiceResult<void> {
    this.log('info', '更新智能体分类', { id: request.id, name: request.name });

    try {
      // 权限校验
      const permissionResult = await this.resolveOperationPermission('AGENT_UPDATE');
      if (!permissionResult.isPermitted) {
        throw this.createAuthorizationError(permissionResult.message || '无权更新智能体分类');
      }
      ...
      if (!existingGroup) {
        throw this.createBusinessError(`智能体分类 ID "${id}" 不存在`);
      }
      ...
      this.log('info', '智能体分类更新成功', { id });
    } catch (error) {
      this.handleServiceError(error, '更新智能体分类');
    }
  }
  ...
  async deleteAgentGroup(request: DeleteAgentGroupRequest): ServiceResult<void> {
    this.log('info', '删除智能体分类', { id: request.id });

    try {
      // 权限校验
      const permissionResult = await this.resolveOperationPermission('AGENT_DELETE');
      if (!permissionResult.isPermitted) {
        throw this.createAuthorizationError(permissionResult.message || '无权删除智能体分类');
      }

      // 检查智能体分类是否存在
      const existingGroup = await this.sessionGroupModel.findById(request.id);
      if (!existingGroup) {
        throw this.createBusinessError(`智能体分类 ID "${request.id}" 不存在`);
      }
      ...
      // 删除智能体分类，分类内智能体的 sessionGroupId 会通过数据库外键约束自动设为 null
      await this.db.delete(sessionGroups).where(and(...conditions));

      this.log('info', '智能体分类删除成功', { id: request.id });
    } catch (error) {
      this.handleServiceError(error, '删除智能体分类');
    }
  }
}
