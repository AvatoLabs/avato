...
- 屏幕与组件：src/screens/*、src/components/ui/*
- 状态管理：src/store/*
...
participant API as "fetchMinkHubApi"
...
### 网络性能与流式传输
- HTTP 封装：统一 fetchMinkHubApi，集中处理 Content-Type 与错误日志，便于后续接入拦截器与重试。
...
- 网络请求失败
  - 查看 fetchMinkHubApi 的错误日志与状态码；必要时增加重试与降级提示。
- 内存占用高
