...
## 更新摘要
**变更内容**
- 新增API清单显示功能，提供更直观的插件和技能API端点展示
- 新增结构化API端点卡片布局，支持API名称徽章和描述文本显示
- 增强了技能详情屏幕的API端点可视化能力
- 新增开发者模式下的API可视化组件
...
**更新** 新增了API清单显示功能，为插件和技能API端点提供更直观的展示方式，支持结构化的API端点卡片布局，包含API名称徽章和描述文本显示。

该功能支持两种类型的技能：Agent 技能（内置或用户创建）和插件技能（社区 MCP 插件）。用户可以通过技能市场浏览、搜索和安装新的技能，也可以查看已安装技能的详细信息和配置选项。新增的API清单功能使得技能的API端点展示更加直观和易用。
...
**新增** 系统新增了API清单显示功能，为插件和技能API端点提供更直观的展示方式。
...
| 功能特性 | 实现方式 | 用户价值 |
|---------|---------|----------|
| API表格展示 | Ant Design Table组件 | 结构化显示API端点 |
| API卡片布局 | 可展开卡片设计 | 详细展示API参数信息 |
| 搜索过滤 | 实时搜索功能 | 快速定位API端点 |
...
| API清单为空 | 显示空状态 | 检查插件清单，重新加载 | 实现API数据验证 |
...
**更新** 新增的API清单显示功能进一步增强了技能详情屏幕的能力，提供了更直观的插件和技能API端点展示方式。通过结构化的API端点卡片布局和表格展示，用户可以更轻松地理解和使用各种API端点。
...
6. **API可视化**: 新增了直观的API端点展示功能，提升开发体验
...
- **API清单功能**: 新增了结构化的API端点展示和管理功能
- **多布局支持**: 支持表格和卡片两种API清单展示布局
