...
Given('用户进入 Avato 对话页面', async function (this: CustomWorld) {
  ...
  console.log('   📍 Step: 查找 Avato...');
  // Find and click on "Avato" agent in the sidebar/home
  const lobeAIAgent = this.page.locator('text=Avato').first();
  await expect(lobeAIAgent).toBeVisible({ timeout: WAIT_TIMEOUT });

  console.log('   📍 Step: 点击 Avato...');
...
Given('用户已发送消息 {string}', async function (this: CustomWorld, message: string) {
  ...
  // Assistant messages are left-aligned .message-wrapper elements that contain "Avato" title
  ...
  const assistantMessage = this.page.locator('.message-wrapper').filter({
    has: this.page.locator('text=Avato'),
  });
...
Then('回复内容应该可见', async function (this: CustomWorld) {
  const assistantMessage = this.page.locator('.message-wrapper').filter({
    has: this.page.locator('.message-header', { hasText: /Avato|AI/ }),
  });
      ...
      async () => {
        const rawText =
          (await assistantMessage
            ...
            .innerText()
            .catch(() => '')) || '';
        finalText = rawText.replaceAll(/Avato/gi, '').replaceAll(/[·•]/g, '').trim();
