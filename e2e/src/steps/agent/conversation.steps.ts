...
Given('用户进入 Mink AI 对话页面', async function (this: CustomWorld) {
  ...
  console.log('   📍 Step: 查找 Mink AI...');
  // Find and click on "Mink AI" agent in the sidebar/home
  const lobeAIAgent = this.page.locator('text=Mink AI').first();
  await expect(lobeAIAgent).toBeVisible({ timeout: WAIT_TIMEOUT });

  console.log('   📍 Step: 点击 Mink AI...');
...
Given('用户已发送消息 {string}', async function (this: CustomWorld, message: string) {
  ...
  // Assistant messages are left-aligned .message-wrapper elements that contain "Mink AI" title
  ...
  const assistantMessage = this.page.locator('.message-wrapper').filter({
    has: this.page.locator('text=Mink AI'),
  });
...
Then('回复内容应该可见', async function (this: CustomWorld) {
  const assistantMessage = this.page.locator('.message-wrapper').filter({
    has: this.page.locator('.message-header', { hasText: /Mink AI|AI/ }),
  });
